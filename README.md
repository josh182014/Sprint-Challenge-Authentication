...
We use sessions for persistance with authentication information so we do not have to always end credentials on every single request that requires them.

1. What does bcrypt do to help us store passwords in a secure manner.

bcrypt gives us a convenient way to hash passwords so they are not store in plain text on our database. They also allow us to check passwords against hashed passwords stored in our database so the user can login.

1. What does bcrypt do to slow down attackers?

bcrypt adds a time aspect to our hashing algorithm. It will slow down attackers since we decide how many times it is going to hash the password.
